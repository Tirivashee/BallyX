...
import type { Customer, SavedItem } from "@/lib/invoice/invoice-data";
...
export function InvoiceForm({ data, onChange, customers = [], savedItems = [] }: Props) {
    ...
    set({
      items: [
        ...data.items,
        {
          ...
          quantity: 1,
          unitCents: savedItem.unitCents,
        },
      ],
    });

  const applyCustomer = (customer: Customer) =>
    setBillTo({ name: customer.name, addressLines: customer.addressLines });
  ...
  return (
    <div className={styles.panel}>
      ...
      <div className={styles.section}>
        ...
        {customers.length > 0 && (
          <div className={styles.field}>
            <label className={styles.label}>Existing customer</label>
            <select
              className={styles.input}
              value=""
              onChange={(e) => {
      ...
      <div className={styles.section}>
        ...
        {savedItems.length > 0 && (
          <div className={styles.field} style={{ marginTop: 10 }}>
            <label className={styles.label}>Or insert a saved item</label>
            <select
              className={styles.input}
              value=""
              onChange={(e) => {
                const savedItem = savedItems.find((it) => String(it.id) === e.target.value);
                if (savedItem) insertSavedItem(savedItem);
              }}
            >
              <option value="">Select a saved item…</option>
              {savedItems.map((it) => (
                <option key={it.id} value={it.id}>
                  {it.name} — {data.currencySymbol}
